...
export
new : (key -> key -> Bool) ->
      (key -> Int) -> Nat -> JVM_IO (IOHash key valInt)
new eq hash n
...
export
insert : key -> value -> IOHash key value -> JVM_IO ()
insert k v table
...
export
update : key -> value -> IOHash key value -> JVM_IO ()
update k v table
    = do let arr = buckets table
         let pos = hash table k `findPos` numBuckets table
         row <- unsafeReadArray arr pos
...
export
delete : key -> IOHash key value -> JVM_IO ()
delete k table
    = do let arr = buckets table
         let pos = hash table k `findPos` numBuckets table
         row <- unsafeReadArray arr pos
...
export
lookup : key -> IOHash key value -> JVM_IO (Maybe value)
lookup k table
    = do let arr = buckets table
         let pos = hash table k `findPos` numBuckets table
         row <- unsafeReadArray arr pos
